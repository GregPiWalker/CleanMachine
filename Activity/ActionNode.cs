...
namespace Sequentials
{
    public class ActionNode : BehavioralState
    {
        ...
        /// <summary>
        /// Gets a collection of CONTINUE Links from the previous action node to this node.
        /// </summary>
        internal List<Link> InboundLinks { get; } = new List<Link>();

        public Link AddEntryLink(IConstraint constraint, IEnumerable<TriggerBase> triggers = null)
        {
            var link = new Link(_context, Stereotypes.Continue.ToString(), this, _logger, AbortToken);
